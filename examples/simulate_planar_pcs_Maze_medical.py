...
from src.planar_pcs_rendering_medical import draw_image
...
def soft_robot_with_safety_contact_CBFCLF_example():
    ...
    class SoRoConfig(CLFCBFConfig):
        ...
        def __init__(self):
            ...
            self.circle_obs_1_radius = robot_radius*2
            self.circle_obs_2_pos = jnp.array([char_length*1.2, char_length*2])
            self.circle_obs_2_radius = robot_radius*1.7
        ...
        def V_2(self, z, z_des) -> jnp.ndarray:
            ...
            error_2 = jnp.sqrt((p_list[1,:] - p_des_list[1,:])**2)*10
            error = jnp.concatenate([error_2, error_1])
    ...
    q0_arary_0 = jnp.array([-jnp.pi*4/3, 0.0, 0.0])
    q0_arary_1 = jnp.array([-jnp.pi*1/3, 0.0, 0.0])
    ...
    ts = jnp.arange(0.0, 7.0, dt) # original is 7
    ...
    fig, axes = plt.subplots(4, 1, figsize=(8,10), sharex=True, num="Regulation example")
