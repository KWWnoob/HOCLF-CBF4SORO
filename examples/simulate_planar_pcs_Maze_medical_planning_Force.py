...
def soft_robot_with_safety_contact_CBFCLF_example():
    ...
    class SoRoConfig(CLFCBFConfig):
        ...
        def __init__(self):
            ...
            self.circle_obs_1_pos = jnp.array([-robot_radius*1.6, char_length])
        ...
        def V_2(self, z, z_des) -> tuple[jnp.ndarray, jnp.ndarray]:
            ...
            error = jnp.concatenate([error_picked*0.4, error_tip]).reshape(-1)
        ...
        def h_2(self, z) -> jnp.ndarray:
            ...
            p_ps_first_segment = p[:20, :2]         # Positions, shape: (N, 2)
            p_ps_second_segment = p[20:, :2]

            
            # -------- Process the polygon obstacles --------
            # inner circle avoid
            tolerance = self.maximum_withhold_force/self.contact_spring_constant
            penetration_inner_first = jax.vmap(compute_distance_circle,in_axes=(0,None))(p_ps_first_segment,self.circle_pos) - self.inner_circle_radius - robot_radius+tolerance
            penetration_outer_first = - jax.vmap(compute_distance_circle,in_axes=(0,None))(p_ps_first_segment,self.circle_pos) + self.outer_circle_radius - robot_radius+tolerance

            penetration_obs_1_first = jax.vmap(compute_distance_circle,in_axes=(0,None))(p_ps_first_segment,self.obs_center[0]) - self.obs_radius[0] - robot_radius+tolerance
            ...
            penetration_inner_second = jax.vmap(compute_distance_circle,in_axes=(0,None))(p_ps_second_segment,self.circle_pos) - self.inner_circle_radius - robot_radius
            penetration_outer_second = - jax.vmap(compute_distance_circle,in_axes=(0,None))(p_ps_second_segment,self.circle_pos) + self.outer_circle_radius - robot_radius

            penetration_obs_1_second = jax.vmap(compute_distance_circle,in_axes=(0,None))(p_ps_second_segment,self.obs_center[0]) - self.obs_radius[0] - robot_radius
            penetration_obs_2_second = jax.vmap(compute_distance_circle,in_axes=(0,None))(p_ps_second_segment,self.obs_center[1]) - self.obs_radius[1] - robot_radius

            safe_distance = jnp.concatenate([penetration_inner_first,penetration_outer_first,penetration_obs_1_first,penetration_obs_2_first,
                                             penetration_inner_second,penetration_outer_second,penetration_obs_1_second,penetration_obs_2_second])
    ...
    for q in q_ts[::20]:
        ...
        img = draw_image(
            ...
            p_des_all = None, #originally was pos
            index = current_index
        )
        img_ts.append(img)
