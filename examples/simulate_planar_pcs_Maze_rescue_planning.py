...
from src.planar_pcs_rendering_rescue_sequential import draw_image
...
def compute_distance(robot, poly, alpha_pair=400., alpha_axes=400.):
...
def smooth_spring_force(d: jnp.ndarray, k: float, eps: float = 1e-4) -> jnp.ndarray:
    """
    Compute smooth spring contact force using scaled softplus.
    
    Args:
        d: Penetration depth (negative if in contact)
        k: Spring constant
    ...
    """
    return k * eps * jnp.log1p(jnp.exp(-d / eps))
    ...
    class SoRoConfig(CLFCBFConfig):
        ...
        def __init__(self):
            ...
            self.poly_obstacle_shape_1 = jnp.array([[0.0, 0.0],
                                                [0.0, char_length*1.8],
                                                [char_length*0.5, char_length*1.8],
                                                [char_length*0.5, 0.0]])
            ...
            self.poly_obstacle_pos_2 = self.poly_obstacle_shape_2 + jnp.array([0.055,0])
            ...
            self.maximum_withhold_force = 100
        ...
        def h_2(self, z) -> jnp.ndarray:
            ...
            force = smooth_spring_force(penetration_depth_poly,self.contact_spring_constant)
            safety = self.maximum_withhold_force - force
            # print(safety.shape)
            return safety
