...
def ideal_point_V2(x_des, y_des, x_obj, y_obj, r_robot, r_obj, max_distance, max_offset = -0.0001, min_offset = 0.0):
    ...
    offset = jnp.where(scale>0.3, max_offset, scale/0.3 * max_offset + (1.0 - scale/0.3) * min_offset )
...
def soft_robot_with_safety_contact_CBFCLF_example():
    ...
    class SoRoConfig(CLFCBFConfig):
        ...
        def V_2(self, z: jnp.ndarray, z_des: jnp.ndarray,
                c0: float = 5.0, c1: float = 1.0) -> jnp.ndarray:
        ...
        def alpha_2(self, h_2):
            return h_2*25 #constant, increase for smaller affected zone
