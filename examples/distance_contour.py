...
plt.plot(mink_shrunk_closed[:,0], mink_shrunk_closed[:,1], '--', color='gray', label='SAT Path',linewidth=2.5,)
for alpha, (cont, col) in contours_dict.items():
    plt.plot(cont[:,0], cont[:,1], label=f'α={alpha}', color=col, linewidth=3.5, alpha=0.7)
...
legend = plt.legend(loc='upper right',  framealpha=0.3)
