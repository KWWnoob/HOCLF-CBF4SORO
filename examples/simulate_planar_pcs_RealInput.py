...
@jax.jit
def compute_distance(robot_vertices, polygon_vertices, alpha=10000):
    rv = robot_vertices
...
def soft_robot_with_safety_contact_CBFCLF_example():
    ...
    class SoRoConfig(CLFCBFConfig):
        ...
        def __init__(self):
            ...
            self.q_des_arary_1 = jnp.array([+jnp.pi*4,0,1.0])
        ...
        # def g(self, z) -> Array:
        #     '''Masking shearing Strain version'''
        #     q, q_d = jnp.split(z, 2)
        #     B, _, _, _, _, _ = dynamical_matrices_fn(self.robot_params, q, q_d)

        #     # Control matrix g(x)
        #     M = jnp.diag(self.actuation_mask)

        #     control_matrix = jnp.linalg.inv(B)
        #     control_matrix = jnp.linalg.inv(B) @ M
        ...
        def g(self, z) -> Array:
            ...
            full_actuation = jnp.linalg.inv(B)  # shape (3, 3)
            reduced_actuation = full_actuation[:, self.active_idx]  # use precomputed concrete index

            zero_block = jnp.zeros((q.shape[0], reduced_actuation.shape[1]))
            matrix = jnp.concatenate([zero_block, reduced_actuation], axis=0) 

            return jnp.concatenate([zero_block, reduced_actuation], axis=0)  # shape (6, m)
        ...
        #     error = jnp.sqrt((self.p_des[:2] - p[-1,:2])**2) * 3e-3
        ...
        def V_2(self, z, z_des) -> jnp.ndarray:
            ...
            q_des = q_des[jnp.array([0, 2])]

            error = jnp.sqrt((q-q_des)**2) # compute the bending and elongation 
            error = error.reshape(-1) 
            # Option: Return the errors as a single vector by concatenating the two.
            # V_total = jnp.concatenate(error)
            return error
    ...
    def closed_loop_ode_fn(t: float, y: Array, q_des: Array) -> Array:
        ...
        q_dd = jnp.linalg.inv(B) @ (u_full - C @ q_d - G - K - D @ q_d)
