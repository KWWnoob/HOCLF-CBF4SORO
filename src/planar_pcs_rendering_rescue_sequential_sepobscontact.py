...
def draw_red_gradient_rectangle_contour_cv2(
    img: onp.ndarray,
    rect: Tuple[int, int, int, int],
    fade_distance: float,
...
#     # Only create color array for mask area
#     mask = (delta <= fade_distance)
#     t = (delta[mask] / fade_distance) ** fade_value
#     b_vals = 1.0 - t
#     r_vals = t
...
#     for d, b, r in zip(ds_masked, b_vals, r_vals):
#         d_mod = d % L
#         if d_mod < w:
#             px, py = x0 + d_mod, y0
#         elif d_mod < w + h:
...
#             (int(px), int(py)),
#             radius=point_radius,
#             color=(int(b * 255), 0, int(r * 255)),  # (B, G, R)
#             thickness=thickness
#         )

# Draw blue→green gradient along full rectangle contour
def draw_blue_gradient_rectangle_contour_cv2(
    img: onp.ndarray,
    rect: Tuple[int, int, int, int],
    fade_distance: float,
    fade_value: float = 1.0,
    ...
    # blue → green (局部蓝色增加，绿色减弱)
    colors[mask, 1] = 1.0 - t        # green fades
    colors[mask, 2] = t  # blue increase
    for d, (b, g, r) in zip(ds, colors):
        d_mod = d % L
        if d_mod < w:
            px, py = x0 + d_mod, y0
...
def draw_image(
    ...
    blue_contact_points: Optional[Array] = None, # polygon number, x, y
    red_contact_points: Optional[Array] = None,
    ...
    if poly_points is not None:
        blue_contact_points_array = blue_contact_points if enable_contact else None
        red_contact_points_array = red_contact_points if enable_contact else None
        ...
        for poly_idx, poly in enumerate(poly_points):
            ...
            if single.shape[0] == 4:
                ...
                contact_list = blue_contact_points_array[poly_idx] if blue_contact_points_array is not None else []
            ...
            if single.shape[0] == 4:
                ...
                contact_list = red_contact_points_array[poly_idx] if red_contact_points_array is not None else []
